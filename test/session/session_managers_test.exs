defmodule ApplicationRunner.SessionManagersTest do
  ...
  test "Can start one Session" do
    assert {:ok, _} = SessionManagers.start_session(make_ref(), make_ref(), %{user: %{}}, %{})
  end
  ...
  test "Can start one session and get it after" do
    ...
    assert {:ok, pid} = SessionManagers.start_session(session_id, env_id, %{user: %{}}, %{})
  ...
  test "Cannot start same session twice" do
    ...
    assert {:error, {:already_started, ^pid}} =
             SessionManagers.start_session(session_id, env_id, %{user: %{}}, %{})
  end
end
